type Mode = "preview" | "run" | "flow";

type UsageEstimate = {
  capsule_token_estimate?: number;
...
const composer = document.querySelector<HTMLFormElement>("[data-composer]");
const conversation = document.querySelector<HTMLElement>("[data-conversation]");
const sessionList = document.querySelector<HTMLElement>("[data-session-list]") || document.createElement("div");
const commandOutput = document.querySelector<HTMLElement>("[data-command-output]") || document.createElement("div");
const pathList = document.querySelector<HTMLElement>("[data-path-list]");
const tokenPanel = document.querySelector<HTMLElement>("[data-token-panel]");
const requirementInput = document.querySelector<HTMLTextAreaElement>("[data-requirement]");
...
modeTabs.forEach((button) => {
  button.addEventListener("click", () => {
    state.selectedMode = button.dataset.mode as Mode;
...
function renderTurn(turn: Turn) {
  ...
  return assistantMessage.querySelector<HTMLElement>(".message-body")!;
}
...
function renderCommand(turn: Turn) {
  ...
  commandOutputEl.textContent = `${prefix}cargo run -- ${command} --profile ${shellQuote(
    turn.profile,
  )} --message ${shellQuote(turn.requirement)}`;
}

async function runWebTurn(turn: Turn): Promise<{ final_answer?: string; usage_estimate?: UsageEstimate }> {
...
function updateAssistantTurn(container: HTMLElement, turn: Turn) {
  const meta = container.querySelector<HTMLElement>(".message-meta");
  if (meta) {
    meta.innerHTML = `
  ...
  const summary = container.querySelector<HTMLElement>(".turn-summary");
  if (summary) {
...
function renderUsageEstimate(usage?: UsageEstimate) {
  if (!usage) {
    return "";
  }
  const chips = [
...
function renderTokenRow(label: string, value?: number) {
  return `
    <div class="token-row">
...
function updatePath(turn?: Turn) {
  const labels =
    (turn?.mode ?? state.selectedMode) === "flow"
      ? [
          "Capture requirement",
